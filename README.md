...
| 風險圖表（CVSS / EPSS / VPR） | **四象限**：縱軸固定 CVSS v2.0、橫軸可切 EPSS 或 VPR（右上=最優先），可切「一點=弱點 / 一點=主機」，資料點 hover 顯示 CVE；另有**漏洞總數與各嚴重度比較**（基準 vs 當前、含增減量）、嚴重度分佈對比、差異總覽、**Top 風險主機**（依弱點總數，依嚴重度堆疊長條）、**主機風險熱力圖**（可切 Σ VPR / Σ EPSS）。全分頁支援 **IP 多選篩選**（可搜尋、風險/IP 降序、圖表即時更新），每張圖標示**資料來源（基準/當前/對比）** |
